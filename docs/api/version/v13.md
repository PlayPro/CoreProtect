...
Use `LookupOptions.Builder.textStartsWithAny(List<String>)` and
`textStartsWithNone(List<String>)` with `chatLookup`, `commandLookup`, or
`signLookup` to filter text before pagination. `textStartsWithAny` requires a match with at least one prefix;
`textStartsWithNone` rejects a match with any prefix. Both conditions apply together,
and either can be used alone. Sign filters check each line of the logged face;
a match on any line counts. Other lookup types ignore these options.

Each entry is one literal prefix, including leading `-`, spaces and commas.
`%`, `_`, `~` and `*` are not wildcards. Prefixes must contain at least three Unicode
code points; shorter prefixes throw `IllegalArgumentException`. Empty lists impose
no restrictions. Matching uses the same SQL predicates as the command's `f:` option.

```java
LookupOptions options = LookupOptions.builder()
        .time(3600)
        .textStartsWithAny(List.of("/msg Notch ", "/tell Notch "))
        .textStartsWithNone(List.of("/msg Notch secret"))
        .limit(0, 100)
        .build();
List<MessageResult> messages = CoreProtect.commandLookup(options);
```
