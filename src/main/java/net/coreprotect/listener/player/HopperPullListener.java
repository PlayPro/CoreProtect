...
public final class HopperPullListener {
    ...
    private static final Map<TransactionId, PendingPull> pendingPulls = new ConcurrentHashMap<>();

    private HopperPullListener() {
        throw new IllegalStateException("Listener class");
    }

    static void processHopperPull(Location location, String user, Inventory sourceInventory, Inventory destinationInventory, boolean destinationContainer, ItemStack[] destinationContents, Boolean fits, ItemStack item) {
        if (location == null || location.getWorld() == null || sourceInventory == null || destinationInventory == null || destinationContents == null || item == null || item.getAmount() <= 0) {
            return;
        }

        String locationSuffix = HopperTransactionUtils.getLoggingIdSuffix(location);
        String loggingChestId = HopperTransactionUtils.getHopperPullId(locationSuffix);
        ...
        if (!worldConfig.HOPPER_TRANSACTIONS) {
            HopperTransactionUtils.recordItemRemoved(transactionId, movedItem);
            return;
        }
        ...
        PendingPull pending = pendingPulls.compute(transactionId, (key, current) -> {
            ...
            if (value == null) {
                value = new PendingPull(sourceInventory, location.clone(), key, locationSuffix, sourceInventory.getMaxStackSize());
            }
    ...
    static void flushPendingPullsForShutdown() {
        for (Map.Entry<TransactionId, PendingPull> entry : pendingPulls.entrySet()) {
    ...
    private static void capturePendingPull(TransactionId transactionId, PendingPull pending) {
        if (!pendingPulls.remove(transactionId, pending)) {
            return;
        }
    ...
    private static void submitPendingPull(PendingPull pending, Inventory sourceInventory, ItemStack[] checkpoint) {
        ...
        ContainerTransactionDispatcher.submit(pending.location, () -> pending.accumulator.replay(checkpoint, pending.maxStackSize,
                (user, sourceBefore, movedItem) -> InventoryChangeListener.onHopperInventoryInteract(user, sourceInventory, sourceBefore, pending.location, pending.transactionId, pending.locationSuffix, movedItem)));
    }
    ...
    private static final class PendingPull {
        ...
        private PendingPull(Inventory inventory, Location location, TransactionId transactionId, String locationSuffix, int maxStackSize) {
