...
List<ContainerResult> containerLookup(Location location, int time, int radius)
```
...
List<MessageResult> chatLookup(String user, int time, int radius, Location radius_location)
List<MessageResult> commandLookup(String user, int time)
List<MessageResult> commandLookup(String user, int time, int radius, Location radius_location)
```
...
#### `containerLookup(Location location, int time, int radius)`

This will perform a container transaction lookup around a location, similar to running `/co lookup a:container r:<radius>`.

* **location:** The center location to perform the lookup around.
* **time:** Specify the amount of time to lookup. "5" would return results from the last 5 seconds.
* **radius:** Specify the radius to search in the X/Z plane. Set to "0" to perform a single-location lookup.
...
#### `chatLookup(String user, int time, int radius, Location radius_location)`

This will perform a chat message lookup around a location, similar to running `/co lookup a:chat r:<radius>`.

* **user:** The username to perform the lookup on. Use `null` or `#global` to search all users.
* **time:** Specify the amount of time to lookup. "5" would return results from the last 5 seconds.
* **radius:** Specify the radius to search in the X/Z plane. Set to "0" to search the exact location.
* **radius_location:** Specify the center location to search around.
...
#### `commandLookup(String user, int time, int radius, Location radius_location)`

This will perform a command lookup around a location, similar to running `/co lookup a:command r:<radius>`.
...
CoreProtectAPI CoreProtect = getCoreProtect();
if (CoreProtect != null){ // Ensure we have access to the API
  List<ContainerResult> lookup = CoreProtect.containerLookup(location, 60, 5);
  if (lookup != null){
    for (ContainerResult result : lookup){
      int x = result.getX();
      int y = result.getY();
...
CoreProtectAPI CoreProtect = getCoreProtect();
if (CoreProtect != null){ // Ensure we have access to the API
  List<MessageResult> lookup = CoreProtect.commandLookup(null, 60, 5, location);
  if (lookup != null){
    for (MessageResult result : lookup){
