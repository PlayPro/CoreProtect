...
### Message prefix filters

Use `LookupOptions.Builder.includeMessagePrefixes(List<String>)` and
`excludeMessagePrefixes(List<String>)` with `chatLookup`, `commandLookup`, or
`signLookup` to filter text before pagination. Include prefixes match any alternative;
exclude prefixes reject any match and take precedence. Exclusions can be used alone.
Sign filters match only the logged face. Other lookup types ignore these options.
...
LookupOptions options = LookupOptions.builder()
        .time(3600)
        .includeMessagePrefixes(List.of("/msg Notch ", "/tell Notch "))
        .excludeMessagePrefixes(List.of("/msg Notch secret"))
        .limit(0, 100)
        .build();
