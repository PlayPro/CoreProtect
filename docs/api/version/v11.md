...
## Upgrading from API v10

The changes from the previous API version are as follows:

- The following event was extended to include Location (mutable):
```java
CoreProtectPreLogEvent(String user, Location location)
```
- The old constructor remains for backward compatibility and is deprecated:
```java
@Deprecated
