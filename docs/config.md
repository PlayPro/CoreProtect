...
Comments can be added with the `%` character, anything after it will be ignored.

An example blacklist.txt file would look like this:

```text
Notch  % Do not log any action from the creator of Minecraft
#creeper
/help
minecraft:stone
minecraft:creeper
...
*To disable logging for entities CoreProtect v23.4+ is required.*
*For user filtered entries, CoreProtect v23.4+ is required.*