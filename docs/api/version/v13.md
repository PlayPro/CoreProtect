...
Use `LookupOptions.messageFilters(List<String>)` with `chatLookup`, `commandLookup`,
or `signLookup` to apply the same prefix matching as the command's `f:` option before
pagination. Include prefixes match any alternative; entries starting with `-` exclude
matches. Exclusions can be used alone. Sign filters match only the logged face.

Each entry is one literal prefix, including spaces and commas. `%`, `_`, `~` and `*`
are not wildcards. Prefixes must contain at least three Unicode code points, excluding
the leading `-`; shorter prefixes throw `IllegalArgumentException`. An empty list
keeps the default results. Other lookup types ignore this option.

```java
LookupOptions options = LookupOptions.builder()
        .time(3600)
        .messageFilters(List.of("/msg Notch ", "/tell Notch ", "-/msg Notch secret"))
        .limit(0, 100)
        .build();
