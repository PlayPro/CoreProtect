        ...
        try {
            ...
            try (final ResultSet results = statement.executeQuery(query)) {
                while (results.next()) {
                    ...
                    if (resultUser == null) {
                        resultUser = UserStatement.loadName(statement.getConnection(), resultUserId);
                    }

                    String timeAgo = ChatUtils.getTimeSince(resultTime, time, true);

                    if (!found) {
                        result.add(Color.WHITE + "----- " + Color.DARK_AQUA + Phrase.build(Phrase.SIGN_HEADER) + Color.WHITE + " ----- " + ChatUtils.getCoordinates(command, worldId, x, y, z, false, false) + "");
                    }
