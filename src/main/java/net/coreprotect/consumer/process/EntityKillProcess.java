    ...
    static void process(PreparedStatement preparedStmt, PreparedStatement preparedStmtEntities, PreparedStatement preparedStmtEntityKillLinks, int batchCount, int processId, int id, Object object, String user) {
        if (object instanceof Object[]) {
            ...
            if (objectLists != null && objectLists.get(id) != null) {
                EntityKillLogger.log(preparedStmt, preparedStmtEntities, preparedStmtEntityKillLinks, batchCount, user, location, objectLists.remove(id), entityId);
            }
            else if (values.length > 2 && values[2] instanceof String) {
                EntityKillLogger.log(preparedStmt, preparedStmtEntities, batchCount, user, location, (String) values[2], entityId);
            }
        }
    }
