...
- Added `blockLookup(LookupOptions)` for block history at a location, within a radius, in a world, or across all worlds. `blockLookup(Block, LookupOptions)` continues to use the supplied block's coordinates.
...
Block material filters match the logged block material, not the block currently at that location. Including materials returns only matching block events; excluding materials leaves entity events unchanged.
